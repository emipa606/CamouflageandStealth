...
namespace CompCamo
{
	...
	public class CanSee_CamoPostPatch
	{
		...
		{
			if (__result && target != null && seer != null && target.Spawned && seer.Spawned && (target?.Map) != null && (seer?.Map) != null && target.Map == seer.Map && target is Pawn && seer is Pawn && CamoUtility.IsTargetHidden(target as Pawn, seer as Pawn))
			{
				__result = false;
			}
		}
